...
fn render_title_bar(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let title_bar = Paragraph::new(Line::from(titles)).block(
        Block::default()
            .title(title.as_str())
            .borders(Borders::ALL)
...
fn render_status_bar(frame: &mut Frame, area: Rect, app: &App) {
    ...
    let status = Paragraph::new(msg).style(Style::default().fg(Color::DarkGray));
    ...
    let lines = vec![
        ...
        Line::from(format!(
            ...
            if exists {
                ...
                "Not found — run treec --neural-link"
            }
        )),
...
fn render_agents(frame: &mut Frame, area: Rect, app: &App) {
    ...
    if agents_dir.exists() {
        if let Ok(entries) = std::fs::read_dir(&agents_dir) {
            ...
            for name in &names {
                let tasks =
                    read_brain_file_count(&app.root, &format!("agents/{}/tasks.md", name), "- [ ]");
    ...
    for name in &pending {
        items.push(ListItem::new(Line::from(vec![
            ...
            Span::styled(
                "(pending — awaiting skill activation)",
                Style::default().fg(Color::DarkGray),
            ),
        ])));
    }
...
fn render_changelog(frame: &mut Frame, area: Rect, app: &App) {
    let content =
        read_brain_or_placeholder(&app.root, "shared_memory/changelog.md", "(changelog empty)");
    let para = Paragraph::new(content)
    ...
    std::fs::read_to_string(&path)
        ...
        .and_then(|c| {
            c.lines()
                ...
                .map(|l| l.replace("**Role:**", "").trim().to_string())
        })
